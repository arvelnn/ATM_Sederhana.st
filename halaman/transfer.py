...
def render(atm):
    ...
    with st.form("form_transfer"):
        tujuan  = st.text_input("🏦 Rekening Tujuan", placeholder="Masukkan nomor rekening")
        nominal = st.number_input("💰 Nominal Transfer", min_value=0, step=50_000)
        catatan = st.text_input("📝 Catatan (opsional)", placeholder="Contoh: Bayar utang")
        kirim   = st.form_submit_button("Transfer Sekarang", use_container_width=True)
